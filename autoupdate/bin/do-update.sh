...
#
# 对被监控列表(union:自己的 repos + 本机其他 *-autoupdate 的 repos)里每个落后的仓:
...
#       安装时的 symlink pull 完即最新)
#   4) 报告拉到的提交;涉及 hook / SKILL.md / commands 变更则提示重启 CLI
#
# 不锁 PATH:可分发工具,用使用者环境的 git。timeout 可选(macOS 默认没有)。
...
fetch_repo() {
  if command -v timeout >/dev/null 2>&1; then
    timeout 8 git -C "$1" fetch --quiet 2>/dev/null
  else
